...
public class MoveTwoAdapter extends BaseAdapter<Tweet.TweetBean> {
    ...
    private int a=0;
    public MoveTwoAdapter(Context context, int layoutId, List<Tweet.TweetBean> datas) {
    ...
    private void dialog(final Tweet.TweetBean newsbean){
        ...
        myDialog.show();
        myDialog.getWindow().setContentView(R.layout.dialogtwo_moban);
    ...
    private void deleteNews(int uid, int tweetid,String cookie){
        NewsModel model=new NewsModel();
        model.deleteTweet(uid, tweetid, cookie, new MyCallback() {
            @Override
            public void onSuccess(String jsondate) {
                Log.e("a",jsondate);
            }
